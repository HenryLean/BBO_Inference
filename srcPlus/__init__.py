...
from multiprocessing import Process, Queue, Pool, Manager, Lock, current_process
...
class SPSA1(object):
    def __init__(self, fn, init=(.5,0,0), tau=(1, 1), cstr=None):
        self.fn = fn
    ...
    def perturb(self, wdlen, delta, normalize=False):
        x_p = self.x + wdlen * delta
        x_m = self.x - wdlen * delta

        y_p = self.fn(x_p, self.tau[0] + self.tau[1])
        y_m = self.fn(x_m, self.tau[0] + self.tau[1])

        g = (y_p - y_m) / (2 * wdlen) * delta
        m = (y_p + y_m) / 2
        if normalize:
            g = g / np.linalg.norm(g)
        return g, m
    
    def perturb4p(self, kappa, wdlen, delta, normalize=False):
        if kappa <= 1:
            warn('Wrong `kappa`, modified!')
            kappa = 3

        x_p = self.x + wdlen * delta
        x_m = self.x - wdlen * delta
        x_pp = self.x + kappa * wdlen * delta
        x_mm = self.x - kappa * wdlen * delta
        ...
        g = ((y_p - y_m) * kappa**3 - (y_pp - y_mm)) / (2 * kappa * (kappa**2 - 1) * wdlen) * delta
        m = ((y_p + y_m) * kappa**2 - (y_pp + y_mm)) / (2 * (kappa**2 - 1))
        if normalize:
            g = g / np.linalg.norm(g)
        return g, m
    ...
    def update(self, k, sclr, obs, beta=1, epsilon=1, eta=.8):
        self.v = self.v + (beta * (obs[1] - self.m)**2 - self.v) / k**(eta)
        self.m = self.m + (epsilon * obs[1] - self.m) / k
        ...
        # self.path['cache'] = np.append(self.path['cache'], obs[1])


    def update_es(self, k, sclr, obs, beta=1, eta=.8, epsilon=1, gamma=.05):
        self.v = self.v + (beta * (obs[1] - self.m)**2 - self.v) / k**(eta)
        self.m = self.m + (epsilon * obs[1] - self.m) * gamma
        self.x = self.project(self.x - sclr * obs[0] / k)
        self.path['x'] = np.append(self.path['x'], [self.x], axis=0)
        self.path['mu'] = np.append(self.path['mu'], self.m)
        self.path['var'] = np.append(self.path['var'], self.v)
...
class BBOSSI(object):
    ...
    def perturbFW(self, c, u):
        x_p = self.x + c * u

        y_p = self.fn(x_p, np.sum(self.tau))
        y_m = self.fn(self.x, np.sum(self.tau))
...
class Replicator(threading.Thread):
    ...
    def spsasi_fw(self, n, p, init=(0,0,0), tau=(1, 1), cstr=None, sclr=1, zeta=1):
        test = BBOSSI(self.fn, init, tau, cstr)

        for k in range(n):
            u = get_unit_sphere(self.__d)
            obs = test.perturbFW(1/(k+1)**(1/5), u)
            test.update_4p(obs, k+1, k/(k+1), sclr, zeta)
        return test.path
    ...
    def repeatSim(self, cltr, method, R, n, param, tau, cstr, sclr):
        ...
        spsa = {'cs': self.spsasi_cs, '4p': self.spsasi_4p, "fw": self.spsasi_fw}
