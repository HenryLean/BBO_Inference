...
class SPSA1s(Optimizer):
    def __init__(self, act, bounds:dict, hypara=False, *args, **kwargs):
        """
        x: the initial decision
        """
        super(SPSA1s, self).__init__(act, hypara=hypara *args, **kwargs)
        self.bounds = bounds

    def iterate(self, env, pertsize, stepsize, tau, **kwargs):
        u = get_unitsphere(self.dim) if self.hypara else np.random.choice((-1,1), size=self.dim)
        obs = [env.get_obs(self.act + i * u * pertsize, np.sum(tau)) for i in (0, 1)]
        gradient = np.mean(obs[1] - obs[0]) / pertsize * u
        self.update(gradient, stepsize, self.bounds, **kwargs)
        return self.act, obs[:1]
